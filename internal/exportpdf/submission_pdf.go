...
func BuildSubmissionsPDF(brandName, formTitle string, submissions []Submission) ([]byte, error) {
	...
	pdf.SetHeaderFunc(func() {
		drawHeader(pdf, logoName, brandName, formTitle, exportedAt)
		drawContentFrame(pdf) // subtle border framing the content area
	})

	pdf.SetFooterFunc(func() {
		drawFooter(pdf, brandName)
	})
...
func drawHeader(pdf *gofpdf.Fpdf, logoName string, brandName string, formTitle string, exportedAt time.Time) {
	const (
		headerTop = 10.0
		leftX     = 12.0
		rightX    = 198.0
	...
	pdf.SetFont("DejaVu", "B", 11)
	pdf.CellFormat(0, 5.5, safeOneLine(brandName), "", 1, "L", false, 0, "")
	pdf.SetX(leftX + 22)
	pdf.SetFont("DejaVu", "", 9.5)
...
func drawFooter(pdf *gofpdf.Fpdf, brandName string) {
	leftX := 12.0
	rightX := 198.0
	...
	pdf.CellFormat(0, 5, safeOneLine("Generated by "+brandName), "", 0, "L", false, 0, "")
...
func buildRows(s Submission, name, email string, dynamicKeys []string) []kvRow {
	...
	if v := safeOneLine(s.RegistrationCode); v != "" {
		rows = append(rows, kvRow{Key: "Registration code", Value: v})
	}
	...
	for _, k := range dynamicKeys {
		...
		label := strings.TrimSpace(s.FieldLabels[k])
		if label == "" {
			label = prettifyKey(k)
		}
