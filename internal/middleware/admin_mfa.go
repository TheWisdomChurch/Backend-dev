...
func adminSecurityError(c *gin.Context, status int, message string, code string, extra gin.H) {
	payload := gin.H{
...
// RequireApprovedAdmin re-checks the database before admin routes run.
// This blocks stale cookies/JWTs after an account is disabled, downgraded,
// deleted, or moved back into pending approval.
func RequireApprovedAdmin(userRepo repository.UserRepository) gin.HandlerFunc {
	return func(c *gin.Context) {
		if userRepo == nil {
			adminSecurityError(c, http.StatusInternalServerError, "Admin security is not configured.", "admin_security_not_configured", nil)
			return
		}

		rawRole, exists := c.Get("role")
		if !exists {
			adminSecurityError(c, http.StatusUnauthorized, "User role not found.", "session_role_missing", nil)
			return
		}

		sessionRole, _ := rawRole.(string)
		...
		if !ok {
			adminSecurityError(c, http.StatusUnauthorized, "Invalid session.", "invalid_session", nil)
			return
		}
		...
		if !isAdminSecurityRole(dbRole) {
			adminSecurityError(c, http.StatusForbidden, "This account no longer has admin access.", "admin_access_revoked", gin.H{"requiredAction": "login_again"})
			return
		}
		if dbRole != sessionRole {
			adminSecurityError(c, http.StatusForbidden, "Your session role is outdated. Please log in again.", "session_role_mismatch", gin.H{"requiredAction": "login_again"})
			return
		}
		if !user.IsActive || !user.AdminApproved {
			adminSecurityError(c, http.StatusForbidden, "Your admin account is awaiting super-admin approval.", "admin_approval_required", gin.H{
...
func RequireAdminMFA(userRepo repository.UserRepository) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if rawTOTP, exists := c.Get("admin_totp_enabled"); exists {
			...
			if userRepo == nil {
				adminSecurityError(c, http.StatusInternalServerError, "Admin MFA security is not configured.", "admin_mfa_not_configured", nil)
				return
			}
			userID, ok := GetUserIDFromContext(c)
			if !ok {
		...
		if !totpEnabled {
			adminSecurityError(c, http.StatusForbidden, "Admin MFA is required. Enable authenticator app verification to continue.", "admin_mfa_required", gin.H{
				"nextStep":        "/mfa/setup",
				"requiredMethod":  "totp",
